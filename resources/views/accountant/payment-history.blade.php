...
<div class="container mx-auto px-4 py-8">
	<div class="mb-6 flex items-center justify-between">
		<h1 class="text-2xl font-bold text-gray-900">Payment History (Processed)</h1>
		<a href="{{ route('accountant.dashboard') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300">Back to Dashboard</a>
	</div>

	@if ($payments->count() === 0)
		<p class="text-gray-600">No payments processed yet.</p>
	@else
				...
				<tbody class="bg-white divide-y divide-gray-200">
					@foreach ($payments as $payment)
						<tr>
							...
							<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $payment->customer->full_name }}</td>
